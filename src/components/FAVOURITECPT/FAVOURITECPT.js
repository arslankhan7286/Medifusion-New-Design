...
class FAVOURITECPT extends Component {
  constructor(props) {
    super(props);
  ...
  callSave = (e) => {
    const { type, providerID, visitReasonID, cptid } = this.state;
    let dataModel = {};
    ...
    // e.preventDefault();
    axios
      .post(this.url + "SaveMostFavouriteCPT", dataModel, this.config)
      .then((response) => {
        console.log("show response", response.data);
        if (
          response.data ==
          "CPT Code with same Provider,Visit Reason and Type , already exist."
        ) {
          Swal.fire(
            "CPT Already Exist",
            "CPT Already Exists In Favourite CPT List",
            "error"
          );
        }
        if (response.data.id > 0 && this.state.Mode == "delete")
          Swal.fire("CPT Deleted Successfully", "", "success");
      })
      .catch((error) => {
    ...
    axios
      .get(this.url + "FindMostFavouriteCPT/", this.config)
      .then((response) => {
  ...
  async addEventListener(e) {
    ...
    await axios
      .get(
        this.url + "FindCPTbyCode?CPTcode=" + e.target.value.toUpperCase(),
        this.config
      )
      .then((response) => {
    ...
    arr.map((row) => {
      ...
      if (start > -1) {
        ...
        b.setAttribute("title", row.cptCode);
  ...
  optionSelected(e) {
    ...
    // adding row to FormCPTList(CPT table)
          ...
          <div className="col-lg-12">
            ...
            <Table responsive>
              <thead className="cpt-heading">
                <tr>
                  <th className="d-block">CPT code</th>
                  <th>Amount</th>
                  <th>Type</th>
                  <th>Provider</th>
...
export default connect(mapStateToProps, matchDispatchToProps)(FAVOURITECPT);
