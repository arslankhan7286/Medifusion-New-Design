...
class AddPatientAlert extends Component {
  constructor(props){
    ...
    this.state = {
      ...
      saveModel : this.props.AlertListData!=null?this.props.AlertListData:this.saveModel,
      searchPatient : this.props.AlertListData!=null?this.props.AlertListData.patientName: "",
      array : [],
      selectedList : [],
      loading : false,
      data:[],
  ...
  componentDidMount() {
    ...
    axios.post(this.url4 + "GeneralItems",Typesearch,this.config)
      .then((response) => {
  ...
  async addEventListener(e) {
    ...
    await axios
      .get(this.url2 + "SearchPatients?criteria=A" + e.target.value.toUpperCase(), this.config)
      .then(response => {
    ...
    this.state.array.map(row => {
      ...
      if (start > -1) {
        ...
        b.innerHTML += "<strong>" + row.name.substr(start, val.length) + "</strong>";
    ...
    this.state.array.map(row => {
      if (row.id == id) {
        ...
        this.setState({
          ...
          saveModel : {...this.state.saveModel,
          patientID : row.id
          }
        });
  ...
  saveData = () => {
    ...
    if(this.state.searchPatient == "" || this.state.searchPatient == null){
      ...
      axios.post(
        this.url + "SavePatientAlerts",
      this.state.saveModel,
        this.config
      ).then(response => {
        this.setState({loading : false, data: response.data})

        this.props.AlertListData!=null? Swal.fire("Record Updated Successfully", "", "success"):
  ...
  render() {   
    ...
    return (
      <div>
           <Modal isOpen={this.state.modal} aria-labelledby="contained-modal-title-vcenter"
         style = {{minHeight : "auto"}}
           centered >
             
             <span style={{ float: "right"}} onClick={this.toggle}><img src = {close}/></span>
             <ModalHeader style = {{backgroundColor : "#186790" , marginTop : "50px"}}><div>
         ...
         <select type = "text" className = {AddStyles.FormSelect} 
         
         name="type" id="type" onChange = {this.handleChange} placeholder = "Type">
           ...
           <option value="">Please Select</option>
                ...
                return (
                  ...
                  <option value={items.type}>{items.name}</option>
                )
          ...
          <div  className = "col-lg-12 text-center mt-2">
            <label htmlFor="Type"  className = {AddStyles.FormLabel}> <strong>Date : </strong> </label>
          ...
          <select type = "text" name = "assignedTo" onChange = {this.handleAssign} className = {AddStyles.FormSelect}  id="Assignedto" placeholder = "Assignedto">
            <option value = {this.state.saveModel.assignedTo}>{this.state.saveModel.assignedTo}</option>
          ...
          <textarea  className = {AddStyles.FormTextarea}
           value ={this.state.saveModel.notes}
          onChange = {this.handleChange} id="title" name="notes" rows="5"></textarea>
           
          </div>

          <div className = "col-lg-12 text-center mt-2">
            <div style = {{float : "left" , width : "100%"}}>
            <button className = {AddStyles.CancelButton} onClick = {this.toggle}> Close </button> 
              <button className = {AddStyles.SaveButton} onClick = {this.saveData}> Save </button>
